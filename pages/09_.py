...
成人の常識（脈があれば押さない）が通用しない、最も危険な落とし穴です。小児にとって「徐脈（心拍数低下）」は、心停止の一歩手前（瀕死）のサインです。
**脈拍が60回/分未満」で、かつ「循環不全のサイン（顔色が悪い、反応がない）**があれば、迷わず胸骨圧迫を開始してください。
アクション:
...
アクション:
**肩枕（タオル）**を入れて、耳の穴と肩のラインを平行にします。
**匂いを嗅ぐような姿勢（スニッフィングポジション)  **を作ると、最も空気が通ります。

*【第4位】 AEDは「未就学児」ならパッドを変える（1.5分）
エネルギー減衰の重要性と、ない場合の対応です。
成人の電気ショックのエネルギーは子どもには強すぎます。
アクション:
未就学児（小学校に上がる前）までは、**小児用パッド」または「小児用キー（モード切り替え）**を使用します。
