    ...
    "🤰 妊産婦": [
        {
            ...
            "text": """
            **【ポイント】**
            * 妊産婦の蘇生は、**「母体と胎児、2つの命」を扱う特殊な状況ですが、インストラクターとして最も伝えるべき鉄則は「母体を救うことが、胎児を救う唯一の道である」**という点です。
10〜15分という短い時間で、受講生の行動を変えるための「4つの重要ポイント」を提案します。ホワイトボードにこの4点を書き出しながら進めると効果的です。
            *<bar>【導入】 マインドセット（1分）
...
なぜ必要か（病態）:仰向け（仰臥位）になると、重たい子宮が**下大静脈（IVC）**を押し潰します（仰臥位低血圧症候群）。これでは、いくら胸骨圧迫をしても心臓に血が帰ってこず、ポンプが空回りします。
どうするか（Action）:
...
「呼吸が止まると、秒単位でSpO2が下がります」。気道確保の難易度:ホルモンの影響で気道が浮腫んでいます。
**「挿管チューブは普段より0.5〜1.0mm細いもの（6.0〜6.5mmなど）」**を用意させます。
