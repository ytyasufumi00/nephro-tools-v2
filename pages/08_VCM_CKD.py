...
with st.sidebar.expander("詳細PKパラメータ / Vdの目安", expanded=False):
    vd_pk = st.slider("分布容積 Vd (L/kg)", 0.4, 1.5, 0.7, 0.05)
    kel_factor = st.slider("排泄係数補正", 0.5, 1.5, 1.0, 0.1)
    
    st.markdown("""
    ---
    **💧 分布容積(Vd) 設定の目安**
    VCMは水溶性のため、体水分量（特に細胞外液）の影響を強く受けます。患者の体組成や水分バランスに合わせて手動で調整してください。

    * **👶 乳児 (1歳未満): `0.7 ～ 1.0`** (最大1.2)
      * 細胞外液が極めて多いため、Vdが最大になります。
    * **🧒 幼児・学童: `0.6 ～ 0.9`**
      * 成長に伴い体水分比率が減り、徐々に成人値へ近づきます。
    * **🧑 成人 (標準): `0.4 ～ 0.7`**
      * 通常は `0.7` 前後を初期値としてシミュレーションします。
    * **🧓 高齢者 / 脱水: `0.4 ～ 0.6`**
      * 体水分量・筋肉量の低下によりVdは小さくなる傾向があります。
      * *(※ただし、心不全や腎不全による「浮腫」がある場合は逆に大きくなります)*
    * **🤰 妊婦 / 敗血症・熱傷: `0.8 ～ 1.0` (以上)**
      * 妊婦は循環血液量や羊水の増加により、著しくVdが増大します。
      * 敗血症性ショック等でサードスペースへ水分が移行している重症患者もVdが大きくなります。
      ---
    **⚡ 排泄係数補正 の目安**
    CCrから自動計算される標準的な排泄速度(1.0倍)を微調整します。
    * **`1.0` (標準):** 通常はここからスタートします。
    * **`1.2 ～ 1.5` (ARC等):** 若年者、外傷、敗血症初期など、過大腎クリアランス(ARC)により予測より早く薬が抜ける場合に大きくします。
    * **`0.5 ～ 0.9`:** 予測より排泄が遅延している（血中濃度が高く出やすい）と想定される場合に小さくします。
    """)
...
with st.expander("📚 目標トラフ濃度・MIC および 自動推奨ロジックに関する解説", expanded=True):
    ...
    * **投与間隔:** CCr に応じて 12, 24, 48, 72時間 のいずれかを推奨
    ...
    #### 👶 小児の場合 (生後1ヶ月以上)
    小児は成人と比較して「分布容積（Vd）が大きく、腎排泄（クリアランス）が速い」という特徴があります。詳細PKパラメーターで、分布容積Vd (L/kg)を調整して下さい。乳児であればVd=1.0　小児、学童であれば VD=0.7～0.9
