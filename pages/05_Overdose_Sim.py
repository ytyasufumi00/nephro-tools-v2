...
default_params = {
    ...
    'カリウム': {
        'V1': 0.2, 'V2': 0.4, # Vd 0.6 L/kg (Total Body Water相当)
        'Q': 0.8, 'T1/2': 24.0, # 腎不全では排泄能低下
        'KoA': 900, # 小分子・高除去率
        'dose': 100, # mEq
        'thresholds': {'Hyperkalemia (>5.5)': 5.5, 'Severe (>7.0)': 7.0},
        'unit': 'mEq/L'
    },
    'エチゾラム (対象外、教育用)': {
...
with st.sidebar.expander("薬剤パラメータ詳細設定", expanded=True):
    # ラベルとステップの動的設定
    input_label = "摂取量 (mEq)" if drug_choice == "カリウム" else "摂取量 (mg)"
    input_step = 10.0 if drug_choice == "カリウム" else 100.0
    
    overdose_amount = st.number_input(input_label, value=float(p['dose']), step=input_step)
